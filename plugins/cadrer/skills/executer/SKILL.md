...
**Before the first slice**, note the commit you start on, then read **À faire à la main** in `architecture.md`. Put every unticked *avant la construction* item to the person in one message — what to do, where, why it is needed — and wait until they say it is done. Check each one you can from here without showing a secret's value — the variable is named where **Comptes et secrets** says, the command answers, the account lets you in — and tick it. One that fails: say what you saw, and start no slice until it passes or they cut it. No `architecture.md`: say the affiner step writes it, and go on.
