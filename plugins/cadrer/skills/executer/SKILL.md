...
**Before the first slice**, note the commit you start on — when that slice picks up where a stopped run left it, the commit before its first build instead — then read **À faire à la main** in `architecture.md`. No `architecture.md`: the builds and audits need it — say the affiner step writes where it runs, and stop. Put every unticked *avant la construction* item to the person in one message — what to do, where, why it is needed. A secret's value never passes through this conversation — and a command typed here with `! ` lands in it, with what it prints: give them the page where they paste the value, or the command to run in a terminal of their own outside this window, into the place **Comptes et secrets** names. `! ` only for a command with no value in it and none in what it prints, such as a login that opens a browser. Wait until they say it is done. Check each one you can from here without showing a secret's value — the variable is named where **Comptes et secrets** says, the command answers, the account lets you in — and tick it; one nothing here can check, tick on their word with *confirmé par la personne* after it; commit `architecture.md` as `À faire à la main : <what was checked>`. One that fails: say what you saw, and start no slice until it passes or they cut it.
