...
Run again after an audit says *retour à la tranche*, or when answers in `a-trancher.md` change what gets built: built slices keep their numbers, ticks and lines; change only the line the audit or the answer named — in `spec.md` too when it says so — and cut what needs new work into a slice numbered after the built ones, adding a story to `spec.md` for an answer. After each answer you placed, add `→ <NN>`, the slice it went to. Same review, same check-in.
