...
Run again after an audit says *retour à la tranche*: built slices keep their numbers, ticks and lines; change only the line the audit named — in `spec.md` too when it says so — and cut findings that need new work into a slice numbered after the built ones. Same check-in.
