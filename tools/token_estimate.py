...
exact row (it costs nothing, but sends the text to the API).
    ...
    uv run tools/token_estimate.py plugins/workflow/skills
    uv run tools/token_estimate.py plugins/workflow/skills path/to/cadrer-fr/skills
...
through the CADRER mapping (ideate=choisir, ...) unless --map overrides it.
"""
...
def main() -> None:
    ...
    parser.add_argument("--map", action="append", default=[], metavar="EN=FR",
                        help="pair skill folders by name; replaces the CADRER mapping (repeatable)")
