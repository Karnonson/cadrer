...
Read `builds/<NN>-<slug>/{{idea_file}}` first (a number or name given as argument picks the folder, else the only one, else ask). Its "{{h_still_open}}" list plus the assumption named under "{{h_why_this}}" are your starting frontier — the assumption goes first, since everything rests on it. Interview the person until you share one understanding of what gets built. No code, no plan, no other files — this session ends with one decisions file. {{language_line}}
...
Done when the frontier is empty. Then write `decisions.md` beside it with exactly these headings: **{{h_settled}}** — table: question · answer · why they chose it; **{{h_assumed}}** — anything you filled in yourself, marked as such; **{{h_dropped}}** — what the interview cut, and why. Re-read once for anything vague or two-readable, fix in place, report what changed. Stop; do not offer to spec or build.
